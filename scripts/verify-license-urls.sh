...
# document links, writing scripts/license-urls.tsv.
#
...
# shellcheck source=scripts/generate-third-party-notices.sh disable=SC1091
source "${HERE}/generate-third-party-notices.sh"

REPOS_MAP="${REPOS_MAP:-scripts/module-repos.tsv}"
URLS_OUTPUT="${URLS_OUTPUT:-scripts/license-urls.tsv}"
...
remote_sha() {
    ...
    blob_tmp_file="$(mktemp "${TMPDIR:-/tmp}/k8s-driver-manager-blob.XXXXXX")"
...
main() {
    ...
    verified_urls_tmp_file="$(mktemp "${TMPDIR:-/tmp}/k8s-driver-manager-urls.XXXXXX")"
...

    {
        ...
        printf '# Generated by scripts/verify-license-urls.sh. Each URL was fetched and its\n'
