...
# shellcheck source=scripts/test-helpers.sh disable=SC1091
source "${HERE}/test-helpers.sh"
# shellcheck source=scripts/license-url-lib.sh disable=SC1091
...
assert_eq "github.com/!make!now!just/heredoc" \
    "$(proxy_escape github.com/MakeNowJust/heredoc)" \
    "proxy_escape escapes every capital within a segment"
...
assert_eq "https://github.com/moby/sys" \
    "$(github_repo_from_path github.com/moby/sys/mountinfo)" "github submodule"
...
assert_eq "mountinfo" \
    "$(github_subdir_from_path github.com/moby/sys/mountinfo)" "github subdir"
...
assert_eq "https://github.com/moby/sys" \
    "$(normalize_repo_url 'https://github.com/moby/sys.git')" "strip .git"
assert_eq "https://github.com/foo/bar" \
    "$(normalize_repo_url 'https://github.com/foo/bar/')" "strip trailing slash"

assert_eq "api" \
    "$(derived_subdir sigs.k8s.io/kustomize/api sigs.k8s.io/kustomize)" \
    "subdir from prefix"
