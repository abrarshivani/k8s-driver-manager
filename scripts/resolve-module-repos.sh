...
# writes scripts/module-repos.tsv.
#
...
OUTPUT="${OUTPUT:-scripts/module-repos.tsv}"
...
fetch_retry() {
    ...
    body_tmp_file="$(mktemp "${TMPDIR:-/tmp}/k8s-driver-manager-fetch.XXXXXX")"
...
main() {
    ...
    repos_tmp_file="$(mktemp "${TMPDIR:-/tmp}/k8s-driver-manager-repos.XXXXXX")"
    ...
    # scripts/verify-license-urls.sh dies when an IN-SCOPE module has no entry in
...

    {
        ...
        printf '# Generated by scripts/resolve-module-repos.sh from the module proxy Origin,\n'
